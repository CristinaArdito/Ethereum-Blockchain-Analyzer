...
public class BlockchainParserPar{
	...
	public String getTransactions(String url) throws Exception {
	    ...
	    try {
        	...
        	System.out.println("Impossibile recuperare il numero di transazioni per il blocco: "+url.substring(30, url.length()));
        }
	    return line;
	}
	 ...
	 * @param url - url pagine contenenti le transazioni
	 * @throws Exception
	 */
    public ArrayList<String> getAddresses(String url) throws Exception {
        ...
        try {
        	...
        	System.out.println("Impossibile recuperare gli indirizzi dei contratti per il blocco: "+url.substring(30, url.length()));        	
        }      
        return addresses;
    }
    ...
    public static void main(String[] args) throws Exception {
    	...
    	for(int i = index; i >= index-15; i--) {   
    		// Ottengo il numero totale di transazioni per il blocco specificato
    		pages = Integer.parseInt(bp.getTransactions("http://etherscan.io/txs?block="+i));
    		// Calcolo il numero di pagine in cui sono listate le transazioni del blocco
    		if(pages > 50) {
	    		int rest = pages % 50;
    		...
    		for(int z = 1; z <= pages; z++) {
    			...
    			// Inserisco gli indirizzi in una lista che conterrà tutti gli indirizzi dei contratti del blocco
    		...
    		while(j.hasNext()) {    			
    			threads.add(new Thread(new Runnable() {
    			    ...
    			    public void run() {
    			    	try {
							bp.getOpcode("https://etherscan.io/address/"+this.address+"#code");
						} catch (Exception e) {
							e.printStackTrace();
						}
    			    }
