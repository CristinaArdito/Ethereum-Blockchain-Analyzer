...
public class BlockchainParserPar {

	/*
	 * Hashmap contenente gli opcodes e la rispettiva numerosità.
	 */
	HashMap<String, int[]> opcodes;

	/*
	 * Hashmap contenente per ogni versione la numerosità di ogni opcodes.
	 */
	HashMap<String, HashMap<String, Integer>> versions;

	/*
	 * Lista delle versioni del compilatore Solidity.
	 */
	ArrayList<String> v = new ArrayList<String>();

	public final String driver = "MONGODB";// {"MONGODB", "INTERNET"}
	...
	/**
	 * Costruttore: inizializzazione delle hashmaps.
	 */
	public BlockchainParserPar() {
		this.opcodes = new HashMap<String, int[]>();
	 ...
	 */
	public void createOpcodesMap(HashMap<String, int[]> opcodes) {
	 ...
	 */
	public void createVersionsMap(HashMap<String, HashMap<String, Integer>> versions) {
	...
	/**
	 * Ottiene il numero di transazioni presenti in ogni blocco.
	 * 
	 * @return line - numero delle pagine
	 * @throws Exception
	 */
	public String getTransactions(String url) throws Exception {
		String line = null;
		...
		try {
			...
			System.out.println("Impossibile recuperare il numero di transazioni per il blocco: "
					+ url.substring(30, url.length()));
		}
		return line;
	}
	 ...
	 * @throws Exception
	 */
	public HashMap<String, String> getAddressesAndCode(int blockNumber) throws Exception {
		...
		FindIterable<org.bson.Document> iterable = Utils.query(client.getDatabase("blockchain"), "transactions",
				Filters.eq("number", blockNumber), null, -1);

		for (org.bson.Document doc : iterable) {
			JSONObject json = new JSONObject(doc.toJson());
			for (Object o : json.getJSONArray("transactions")) {

				JSONObject json2 = new JSONObject(o.toString());
				if (json2.getDouble(("value")) == 0) {
					// Here is contract address and the contract creation data
					ret.put(json2.getString("to"), json2.getString("data"));
				}
			}
		}
	...
	/**
	 * Ottiene gli indirizzi dei contratti.
	 * 
	 * @param url
	 *            - url pagine contenenti le transazioni
	 * @throws Exception
	 */
	public HashSet<String> getAddresses(String url) throws Exception {
		...
		try {
			...
			System.out.println("Impossibile recuperare gli indirizzi dei contratti per il blocco: "
					+ url.substring(30, url.length()));
		}
		return addresses;
	}
	...
	public boolean isVerified(String url) throws Exception {
		...
		try {
			...
			System.out.println("Errore nel contratto " + url.substring(29, url.lastIndexOf("#"))
					+ ". Impossibile recuperare la versione del compilatore.");
			e.printStackTrace();
		}
		return verified;
	}
	...
	/**
	 * Ottiene la versione del compilatore Solidity utilizzata e la memorizza su
	 * file
	 * 
	 * @param url
	 *            - url pagine contenenti i contratti
	 * @throws Exception
	 */
	...
	/**
	 * Ottiene gli Opcode tramite il codice sorgente di ogni pagina dei
	 * contratti e li memorizza su file
	 * 
	 * @param url
	 *            - url del contratto
	 * @throws Exception
	 */
	public void getOpcode(String url, HashMap<String, int[]> localOpcodes,
			HashMap<String, HashMap<String, Integer>> localVersions) throws Exception {
		...
		try {
			/*
			 * Eseguo il codice javascript relativo all'evento onclick(), per
			 * accedere alla versione della pagina contenente gli opcodes
			 */
			...
			if (page != null) {
				...
				while ((inputLine = in.readLine()) != null) {
					...
					if (inputLine.contains("fa fa-database") == true || inputLine.contains("readContract") == true
							|| inputLine.contains("fa fa-plus-square") == true) {
						flag = false;
					}
					i++;
					if (flag == true && i > 1) {
						if (inputLine.contains("<br/>") == false && inputLine.contains("</div>") == false
								&& inputLine.contains("</pre>") == false && inputLine.contains("wordwrap") == false) {
							if (verified)
								startIndex = 24;
							else
								startIndex = 22;
							...
							/*
							 * Se il contratto risulta verificato incremento il
							 * relativo counter
							 */
							if (verified) {
								if (localOpcodes.containsKey(opcode)) {
									int array[] = localOpcodes.get(opcode);
									array[1] += 1;
									localOpcodes.replace(opcode, array);
								}
							} /*
							...
							if (!version.isEmpty()) {
								/*
								 * Incremento il counter dell'opcode per la
								 * versione specificata
								 */
								if (localVersions.containsKey(opcode)) {
									HashMap<String, Integer> map = localVersions.get(opcode);
									if (version.contains("-"))
									...
									if (map.containsKey(version)) {
										...
										localVersions.replace(opcode, map);
									}
									...
									/*
									 * Altrimenti incremento il counter per
									 * contratti prodotti da altri linguaggi
									 */
			...
			System.out.println("Errore nel contratto " + url.substring(29, url.lastIndexOf("#"))
					+ ". Impossibile recuperare gli Opcodes.");
			e.printStackTrace();
		}
	}
	...
	/**
	 * Memorizza i risultati della statistica degli opcodes per ogni versione su
	 * file.
	 * 
	 * @throws Exception
	 */
	...
	public static void main(String[] args) throws Exception {
		BlockchainParserPar bp = new BlockchainParserPar();
		HashSet<String> addresses = new HashSet<String>();
		HashSet<String> addressesApp = new HashSet<String>();
		ArrayList<Thread> threads = new ArrayList<Thread>();

		//uso locale
		ArrayList<HashMap<String, int[]>> localsOpcodes = new ArrayList<HashMap<String, int[]>>();
		ArrayList<HashMap<String, HashMap<String, Integer>>> localsVersions = new ArrayList<HashMap<String, HashMap<String, Integer>>>();

		int pages = 0;
		// Creo l'hashmap per gli opcodes
		bp.createOpcodesMap(bp.opcodes);
		// Inizializzo le versioni del compilatore da analizzare
		bp.insertVersions();
		// Creo l'hashmaps con un opcode per ogni versione
		bp.createVersionsMap(bp.versions);
		// Ottengo il numero totale ti blocchi presenti nella Blockchain
		// int index = Integer.parseInt(Utils.getBlocksNumber());

		System.out.println("Blocco corrente: " + bp.maxBlock);
		...
		for (int i = bp.maxBlock; i >= bp.minBlock; i--) {
			if (bp.driver.equalsIgnoreCase("INTERNET")) {
				// Ottengo il numero totale di transazioni per il blocco
				// specificato
				pages = Integer.parseInt(bp.getTransactions("http://etherscan.io/txs?block=" + i));
				// Calcolo il numero di pagine in cui sono listate le
				// transazioni del blocco
				if (pages > 50) {
					int rest = pages % 50;
					if (rest > 0)
						pages = (pages / 50) + 1;
				...
				for (int z = 1; z <= pages; z++) {
					// Ottengo gli indirizzi dei contratti di ogni pagina
					addressesApp = bp.getAddresses("http://etherscan.io/txs?block=" + i + "&p=" + z);
					// Inserisco gli indirizzi in una lista che conterrà tutti
					// gli indirizzi dei contratti del blocco
				...
				// Creo un thread per ogni contratto
				...
				while (j.hasNext()) {
					threads.add(new Thread(new Runnable() {
						...
						public void run() {
							try {
								HashMap<String, int[]> localOpcodes = new HashMap<String, int[]>();
								bp.createOpcodesMap(localOpcodes);
								HashMap<String, HashMap<String, Integer>> localVersions = new HashMap<String, HashMap<String, Integer>>();
								bp.createVersionsMap(localVersions);
								bp.getOpcode("https://etherscan.io/address/" + this.address + "#code", localOpcodes,
										localVersions);
								localsOpcodes.add(localOpcodes);
								localsVersions.add(localVersions);
							} catch (Exception e) {
								e.printStackTrace();
							}
						}
				...
				System.out.println(localsOpcodes.size());
				System.out.println(localsVersions.size());
				for (HashMap<String, int[]> app : localsOpcodes) {
					for (String key : app.keySet()) {
						int globalArray[] = bp.opcodes.get(key);
						int localArray[] = app.get(key);
						globalArray[0] += localArray[0];
						globalArray[1] += localArray[1];
						globalArray[2] += localArray[2];
						globalArray[3] += localArray[3];
						bp.opcodes.replace(key, globalArray);
					}

				}
				for (HashMap<String, HashMap<String, Integer>> app : localsVersions) {
					for (String opcode : app.keySet()) {
						for (String version : app.get(opcode).keySet())
							bp.versions.get(opcode).replace(version,
									bp.versions.get(opcode).get(version) + app.get(opcode).get(version));

					}

				}

			} else if (bp.driver.equalsIgnoreCase("MONGODB")) {
				System.out.println("Getting addresses....");
				HashMap<String, String> ret = bp.getAddressesAndCode(i);
				System.out.println("... got " + ret.size() + "addresses.");

				for (String address : ret.keySet()) {
					bp.getOpcode("https://etherscan.io/address/" + address + "#code", bp.opcodes, bp.versions);
				}
			}
			// Effettuo un backup dei dati del blocco su disco
			bp.backup(i);
			addresses.clear();
			threads.clear();
		...
		bp.writeOpcodesResults(bp.path + "OpcodesResults.txt");
		bp.writeVersionsResults(bp.path + "VersionsResults.txt");
	}
